    ...
    <div className="absolute top-0 w-full z-50 p-4 md:p-6 pointer-events-none">
      <nav className="pointer-events-auto max-w-7xl mx-auto bg-white dark:bg-slate-900 border-[2px] border-black dark:border-white py-3 px-6 rounded-xl shadow-xl flex items-center justify-between transition-all">
        ...
        <div className="hidden md:flex items-center gap-3">
          {links.map((link) => {
            if (link.path === '/submit-idea') {
              const isActive = currentPath === link.path;
              return (
                <button
                  key={link.path}
                  onClick={() => onNavigate(link.path)}
                  className={`px-5 py-2 border-[2px] rounded-xl font-black text-xs uppercase transition-all flex items-center gap-1.5 border-black dark:border-white
                    ${isActive
                      ? 'bg-[#FFDE03] text-black shadow-[4px_4px_0px_0px_#000000] dark:shadow-[4px_4px_0px_0px_#FFFFFF] ring-2 ring-black dark:ring-white scale-105'
                      : 'bg-[#FFDE03] text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] hover:bg-[#ffe633] hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:hover:shadow-[6px_6px_0px_0px_rgba(255,255,255,1)] active:shadow-none active:translate-x-1 active:translate-y-1'}`}
                >
                  <Lightbulb size={14} className="fill-black text-black" />
            ...
            if (link.path === '/submit-idea') {
              return (
                <button
                  ...
                  className="w-full text-center px-6 py-3 border-[2px] rounded-lg border-black dark:border-white font-black text-sm uppercase transition-all flex items-center justify-center gap-2 bg-[#FFDE03] text-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_rgba(255,255,255,1)]"
                >
                  <Lightbulb size={16} className="fill-black text-black" />
                  <span>{link.label}</span>
                </button>
              );
            }
